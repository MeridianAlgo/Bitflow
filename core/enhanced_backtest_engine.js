...
class EnhancedBacktestEngine {
    ...
    async runBacktest(ohlcvData, config = {}) {
        const MIN_CONSOLE_OUTPUT = process.env.MIN_CONSOLE_OUTPUT === '1';
        
        if (!MIN_CONSOLE_OUTPUT) {
    ...
    saveResults(outputDir = './logs/backtest_results') {
        ...
        // Save to learning database for continuous improvement
        this.saveToLearningDatabase(report);
        
        console.log(`Results saved to ${jsonPath} and ${csvPath}`);
        return { jsonPath, csvPath };
    }
    
    // Save results to learning database for continuous improvement
    saveToLearningDatabase(report) {
        const learningDbPath = path.join('./logs/learning_database');
        if (!fs.existsSync(learningDbPath)) {
            fs.mkdirSync(learningDbPath, { recursive: true });
        }
        
        // Save symbol-specific performance data
        const symbolDbPath = path.join(learningDbPath, `${this.symbol.replace('/', '')}.json`);
        let symbolData = {};
        
        // Load existing data if available
        if (fs.existsSync(symbolDbPath)) {
            try {
                symbolData = JSON.parse(fs.readFileSync(symbolDbPath, 'utf8'));
            } catch (err) {
                console.log(`Error reading learning database: ${err.message}. Creating new database.`);
            }
        }
        
        // Update with new backtest data
        const timestamp = new Date().toISOString();
        symbolData.lastUpdated = timestamp;
        symbolData.backtestCount = (symbolData.backtestCount || 0) + 1;
        
        // Store performance metrics history
        if (!symbolData.performanceHistory) {
            symbolData.performanceHistory = [];
        }
        
        // Keep only the last 10 backtest results to avoid excessive file size
        if (symbolData.performanceHistory.length >= 10) {
            symbolData.performanceHistory.shift(); // Remove oldest entry
        }
        
        // Add new performance data
        symbolData.performanceHistory.push({
            timestamp,
            metrics: this.performanceMetrics,
            totalReturn: report.totalReturn,
            finalBalance: report.finalBalance,
            tradeCount: this.trades.length,
            bestParameters: report.parameters || {}
        });
        
        // Calculate aggregated metrics for strategy improvement
        symbolData.aggregatedMetrics = this.calculateAggregatedMetrics(symbolData.performanceHistory);
        
        // Save updated data
        fs.writeFileSync(symbolDbPath, JSON.stringify(symbolData, null, 2));
        console.log(`Learning database updated at ${symbolDbPath}`);
    }
    
    // Calculate aggregated metrics from historical backtest data
    calculateAggregatedMetrics(history) {
        if (!history || history.length === 0) return {};
        
        // Extract metrics from history
        const metrics = {
            winRate: history.map(h => h.metrics.winRate).filter(Boolean),
            profitFactor: history.map(h => h.metrics.profitFactor).filter(Boolean),
            sharpeRatio: history.map(h => h.metrics.sharpeRatio).filter(Boolean),
            maxDrawdownPercent: history.map(h => h.metrics.maxDrawdownPercent).filter(Boolean),
            totalReturn: history.map(h => h.totalReturn).filter(Boolean)
        };
        ...
        return {
            avgWinRate: calculateAvg(metrics.winRate),
            avgProfitFactor: calculateAvg(metrics.profitFactor),
            avgSharpeRatio: calculateAvg(metrics.sharpeRatio),
            avgMaxDrawdown: calculateAvg(metrics.maxDrawdownPercent),
            avgTotalReturn: calculateAvg(metrics.totalReturn),
            improvementTrend: this.calculateImprovementTrend(history),
            recommendedParameters: this.generateRecommendedParameters(history)
        };
    }
    
    // Calculate improvement trend over time
    calculateImprovementTrend(history) {
        if (history.length < 2) return { improving: false, trend: 0 };
        
        // Use total return as the primary metric for improvement trend
        ...
        return {
            improving: trend > 0,
            trend: trend,
            percentImprovement: firstHalfAvg !== 0 ? (trend / Math.abs(firstHalfAvg)) * 100 : 0
        };
    }
    
    // Generate recommended parameters based on historical performance
    generateRecommendedParameters(history) {
        if (history.length < 2) return {};
        
        // Sort by performance (total return)
        const sortedHistory = [...history].sort((a, b) => b.totalReturn - a.totalReturn);
        
        // Get parameters from top 3 performing backtests
        const topPerformers = sortedHistory.slice(0, Math.min(3, sortedHistory.length));
        ...
        topPerformers.forEach(performer => {
            if (performer.bestParameters) {
                Object.keys(performer.bestParameters).forEach(key => paramKeys.add(key));
            }
        });
        ...
        paramKeys.forEach(key => {
            ...
            topPerformers.forEach((performer, index) => {
                if (performer.bestParameters && performer.bestParameters[key] !== undefined) {
                    // Weight by performance and recency (higher index = more recent)
                    const weight = performer.totalReturn * (index + 1);
                    weightedSum += performer.bestParameters[key] * weight;
                    weightSum += weight;
                }
            });
            
            if (weightSum > 0) {
                recommendedParams[key] = weightedSum / weightSum;
            }
        });
