...
// Replacement for fetchPolygonNews - uses Google News instead
async function fetchPolygonNews(symbol, limit = 5) {
    try {
...
// Replacement for checkPolygonNewsAPI - checks Google News API instead
async function checkPolygonNewsAPI() {
    try {
        // Test the Google News API with a generic query
        const testArticles = await fetchGoogleNewsHeadlines('cryptocurrency');
        return testArticles.length > 0;
    } catch (error) {
        console.warn('Error checking news API:', error.message);
        return false;
    }
}

// Check crypto market status without using Polygon
async function checkCryptoMarketStatus() {
    try {
        // Crypto markets are generally 24/7, so we'll return true
        // In a production environment, you might want to check for maintenance windows
        return {
            open: true,
            message: 'Crypto market is OPEN (24/7)',
            error: null
        };
    } catch (error) {
        return {
            open: false,
            message: 'Unable to determine crypto market status',
            error: error.message
        };
    }
}
